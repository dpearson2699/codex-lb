...
equal the legacy raw `COUNT(*)`. Signatures carrying free-text search,
error-code splits or a request-log `source` filter (the rollups carry no
`source` dimension) MUST fall back to the exact raw count. The cache key MUST
distinguish signatures that differ only in their `source` filter.
...
#### Scenario: A source filter falls back to the raw count
- **GIVEN** a listing filtered by request-log `source`
- **WHEN** the total is computed
- **THEN** the exact raw `COUNT(*)` path MUST be used
- **AND** the demand rollup MUST NOT be read
- **AND** the same listing without the `source` filter MUST still take the rollup path
